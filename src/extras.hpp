...
static std::vector<char> injectExtrasIntoGlbData(
    ...
    if (!shapes.empty() && doc.HasMember("meshes") && doc["meshes"].IsArray()) {
    ...
    // Add materials to root extras
    if (materials != nullptr && materials->IsArray() && materials->Size() > 0) {
        // Ensure asset.extras exists
        if (!doc.HasMember("asset")) {
            doc.AddMember("asset", rapidjson::Value(rapidjson::kObjectType), doc.GetAllocator());
        }
        if (!doc["asset"].HasMember("extras")) {
            doc["asset"].AddMember("extras", rapidjson::Value(rapidjson::kObjectType), doc.GetAllocator());
        }
        
        // Deep copy materials array
        rapidjson::Value materialsCopy;
        materialsCopy.CopyFrom(*materials, doc.GetAllocator());
        doc["asset"]["extras"].AddMember("materials", materialsCopy, doc.GetAllocator());
    }
...
static bool injectExtrasIntoGlb(const char* glbPath, 
    ...
    // Process in memory
    std::vector<char> result = injectExtrasIntoGlbData(glbData, shapes, allowedTypes, materials);
    if (result.empty()) {
        return false;
    }
